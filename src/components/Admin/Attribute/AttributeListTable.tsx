...
export default function AttributeListTable({ items }: { items: Item[] }) {
	return (
					...
					{items.map((item) => (
						<tr key={item.id} className="border-b border-stroke last:border-0 dark:border-stroke-dark">
							<td className="p-4 text-body dark:text-gray-5 sm:pl-7.5">
								...
								<span className="block lg:hidden text-sm text-body/80">{item.data_type}</span>
								<span className="block sm:hidden"><AttributeAction item={item} /></span>
							</td>
							<td className="hidden p-4 text-body dark:text-gray-5 md:table-cell">{item.name_en}</td>
							<td className="hidden p-4 text-body dark:text-gray-5 lg:table-cell">{item.data_type}</td>
