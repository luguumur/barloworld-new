...
export default function QuoteRequestAction({
	quote,
}: {
	...
	const handleStatusToggle = async () => {
		try {
			const next = STATUS_CYCLE[quote.status] ?? "NEW";
			await updateQuoteRequestStatus(quote.id, next);
			router.refresh();
		} catch {
	...
	return (
		<>
			...
			<DeleteModal
				...
				deleteText='Remove Quote Request'
