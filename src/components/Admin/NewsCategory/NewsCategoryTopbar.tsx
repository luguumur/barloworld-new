...
export default function NewsCategoryTopbar({
	onAddClick,
	initialSearch = "",
}: {
	onAddClick: () => void;
	...
	return (
		<div className='items-center justify-between rounded-10 bg-white px-3.5 py-3 shadow-1 dark:bg-gray-dark md:flex'>
			<div className='mb-6 flex flex-wrap items-center gap-3 md:mb-0'>
				<button
					onClick={onAddClick}
					className='flex h-10 items-center justify-center gap-3 rounded-lg bg-primary p-3 text-sm text-white hover:bg-primary-dark'
				>
					<Image
			...
			<div className='flex flex-wrap items-center gap-3'>
				<form
					onSubmit={(e) => {
						e.preventDefault();
						router.push(
							`/admin/news-category?search=${encodeURIComponent(search)}`
						);
					}}
				>
					<div className='relative'>
						<input
							...
							className='h-11 w-full rounded-lg border border-stroke bg-gray-1 pl-11 pr-4.5 text-sm outline-none ring-offset-1 duration-300 focus:shadow-input focus:ring-2 focus:ring-primary/20 dark:border-stroke-dark dark:bg-transparent dark:focus:border-transparent'
							value={search}
