...
import { deleteHomeCard, reorderHomeCards, HomeCardRow } from "@/actions/homeMain";
...
function SortableRow({ card }: { card: HomeCardRow }) {
	...
	return (
		<>
			<tr ref={setNodeRef} style={style} className='border-b border-stroke last-of-type:border-b-0 dark:border-stroke-dark'>
				{/* drag handle */}
				...
				<td className='p-3'>
					<div className='h-12 w-16 overflow-hidden rounded-lg border border-stroke dark:border-stroke-dark'>
						{src ? (
							<img src={src} alt={card.title_en} className='h-full w-full object-cover' />
						) : (
				...
				<td className='p-3'>
					<p className='font-medium text-dark dark:text-white'>{card.title_en}</p>
					...
					<div className='flex items-center justify-end gap-2'>
						...
						>
							<svg width='18' height='18' viewBox='0 0 21 20' fill='none'>
								<path fillRule='evenodd' clipRule='evenodd' d='M9.53 1.875h2.82c.37 0 .71.17.93.46.07.13.12.28.18.45l.09.28c.15.4.53.67.96.69h2.5c.35 0 .63.28.63.625s-.28.625-.63.625H3.86a.625.625 0 0 1 0-1.25h2.5c.43-.02.81-.29.96-.69l.09-.28c.06-.17.11-.32.18-.45.22-.29.56-.46.93-.46Z' fill='currentColor' />
								<path d='M5.87 7.042a.625.625 0 0 0-1.247.083l.387 5.793c.07 1.069.13 1.932.264 2.61.14.704.38 1.292.872 1.753.493.461 1.096.66 1.808.753.685.09 1.55.09 2.622.09h.742c1.071 0 1.937 0 2.622-.09.712-.093 1.315-.292 1.808-.753.493-.461.731-1.049.872-1.752.134-.679.193-1.542.264-2.61l.387-5.794a.625.625 0 0 0-1.247-.083l-.384 5.749c-.075 1.123-.128 1.905-.245 2.493-.114.57-.272.872-.5 1.085-.228.213-.54.351-1.116.427-.594.077-1.378.078-2.503.078h-.635c-1.125 0-1.91-.001-2.503-.078-.577-.076-.888-.214-1.116-.427-.228-.213-.386-.515-.5-1.085-.117-.588-.17-1.37-.245-2.493L5.87 7.042Z' fill='currentColor' />
							</svg>
						</button>
					</div>
				</td>
		...
		<div>
			...
			{items.length ? (
					...
					<table className='w-full'>
						<thead>
							<tr className='border-b border-stroke dark:border-stroke-dark'>
								...
								<th className='p-3 text-left text-sm font-medium text-body dark:text-gray-5'>Image</th>
								<th className='p-3 text-left text-sm font-medium text-body dark:text-gray-5'>Title</th>
								<th className='hidden p-3 text-left text-sm font-medium text-body dark:text-gray-5 md:table-cell'>URL</th>
								<th className='hidden p-3 text-center text-sm font-medium text-body dark:text-gray-5 sm:table-cell'>Order</th>
								<th className='p-3 text-right text-sm font-medium text-body dark:text-gray-5 sm:pr-6'>Action</th>
							</tr>
						</thead>
				...
				<div className='rounded-10 bg-white py-12 text-center shadow-1 dark:bg-gray-dark'>
					<p className='text-body dark:text-gray-5'>No cards yet. Add up to 6.</p>
				</div>
			)}
		</div>
	);
