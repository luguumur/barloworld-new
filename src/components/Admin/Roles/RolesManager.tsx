...
const ALL_PERMISSIONS: { path: string; label: string; group: string }[] = [
	...
	{ path: "/admin/product-categories", label: "Product Categories", group: "Catalog" },
	{ path: "/admin/attribute-value-groups", label: "Attribute Groups", group: "Catalog" },
...
function RoleBadge({ color, label }: { color: string; label: string }) {
	return (
		<span className='inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium text-white' style={{ background: color }}>
			{label}
		</span>
	);
}

export default function RolesManager({ roles: initial }: { roles: AdminRoleRow[] }) {
	...
	const [newRole, setNewRole] = useState({ name: "", label: "", color: COLORS[0], permissions: [] as string[] });
	...
	const handleUpdate = () => {
		...
		startTransition(async () => {
			try {
				...
				setRoles((r) => r.map((role) => (role.id === updated.id ? updated : role)));
				setEditing(null);
	...
	const PermissionGrid = ({
		...
		<div className='space-y-4'>
			{groups.map((group) => (
				<div key={group}>
					<p className='mb-2 text-xs font-semibold uppercase tracking-wider text-body dark:text-gray-4'>{group}</p>
	...
	return (
		<div className='space-y-6'>
			{/* Existing roles */}
			{roles.map((role) => (
				<div key={role.id} className='rounded-10 bg-white p-6 shadow-1 dark:bg-gray-dark'>
					{editing?.id === role.id ? (
						<div className='space-y-4'>
							<div className='flex items-center gap-3'>
								<RoleBadge color={editing.color} label={editing.label || "Preview"} />
								<input
									value={editing.label}
								...
								<div className='flex gap-1.5'>
									{COLORS.map((c) => (
										<button
											...
											className={`h-6 w-6 rounded-full transition-transform ${editing.color === c ? "scale-125 ring-2 ring-offset-1 ring-primary" : ""}`}
											style={{ background: c }}
										/>
									))}
								</div>
						...
						<div>
							<div className='mb-4 flex items-center justify-between'>
								<div className='flex items-center gap-3'>
									...
									<span className='text-xs text-body dark:text-gray-4'>{role.name}</span>
								...
								<div className='flex gap-2'>
									...
									<button onClick={() => handleDelete(role.id)} disabled={isPending}
							...
							<div className='flex flex-wrap gap-2'>
								{role.permissions.length === 0 ? (
									<span className='text-sm text-body dark:text-gray-4'>No permissions — this role cannot access any section.</span>
								) : (
									ALL_PERMISSIONS.filter((p) => role.permissions.includes(p.path)).map((p) => (
										<span key={p.path} className='rounded-lg bg-gray-1 px-2.5 py-1 text-xs font-medium text-dark dark:bg-white/5 dark:text-white'>
											{p.label}
										</span>
									))
								)}
			...
			{creating ? (
				<div className='rounded-10 bg-white p-6 shadow-1 dark:bg-gray-dark'>
					<h4 className='mb-4 font-semibold text-dark dark:text-white'>New Role</h4>
					<div className='mb-4 flex items-center gap-3'>
						<input
							value={newRole.name}
							onChange={(e) => setNewRole({ ...newRole, name: e.target.value.toUpperCase().replace(/\s/g, "_") })}
							placeholder='ROLE_KEY (e.g. EDITOR)'
							className='w-40 rounded-lg border border-stroke bg-gray-1 px-3 py-2 text-sm font-mono dark:border-dark-3 dark:bg-white/5 dark:text-white'
						/>
						<input
							value={newRole.label}
						...
						<div className='flex gap-1.5'>
							{COLORS.map((c) => (
								<button key={c} type='button'
									...
									className={`h-6 w-6 rounded-full transition-transform ${newRole.color === c ? "scale-125 ring-2 ring-offset-1 ring-primary" : ""}`}
									style={{ background: c }}
								/>
							))}
						</div>
