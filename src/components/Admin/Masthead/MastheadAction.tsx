...
import { deleteMasthead } from "@/actions/masthead";
...
export default function MastheadAction({ masthead }: { masthead: Masthead }) {
	...
	const handleDelete = async () => {
		setLoading(true);
		try {
			await deleteMasthead(masthead.id);
			toast.success("Masthead deleted successfully!");
			router.refresh();
	...
	return (
		<>
			<div className="mt-2 flex flex-wrap items-center gap-3.5 lsm:ml-auto lsm:justify-end sm:mt-0">
				<Link
					href={`/admin/mastheads/${masthead.id}/edit`}
