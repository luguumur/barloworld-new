...
import { upsertLocationSetting, LocationSettingInput } from "@/actions/locationSetting";
...
export default function LocationSettingForm({ initial }: Props) {
	...
	const set = (key: keyof LocationSettingInput) =>
	...
	const uploadImage = async (): Promise<string | null> => {
		...
		const result = await getSignedURL(file.type, file.size, "location", `${Date.now()}.${ext}`);
	...
	return (
		<form onSubmit={handleSubmit} className='flex flex-col gap-5'>
			...
			<div>
				...
				<div className='flex flex-wrap items-start gap-4'>
					<label className='relative flex h-40 w-64 shrink-0 cursor-pointer items-center justify-center overflow-hidden rounded-lg border border-stroke bg-gray-1 dark:border-stroke-dark dark:bg-white/5'>
						{displayPreview ? (
							<Image src={displayPreview} alt='location' fill className='object-cover' sizes='256px' unoptimized={displayPreview.startsWith("blob:")} />
						) : (
							<span className='text-sm text-body/60'>Click to upload</span>
						)}
						<input type='file' className='sr-only' accept='image/png,image/jpeg,image/jpg' onChange={handleFileChange} />
					</label>
					<div className='flex flex-col gap-2'>
						...
						/>
						{file && (
							<button type='button' onClick={() => { setFile(null); setPreview(data.image || ""); }} className='text-xs text-red hover:underline'>
								Remove file
							</button>
						)}
					</div>
			...
			{/* Map embed URL */}
			<div>
				<label className='mb-2 block font-satoshi text-sm font-medium text-dark dark:text-white'>Google Maps Embed URL</label>
				<textarea
					rows={3}
			...
			<div className='grid gap-4 sm:grid-cols-2'>
				<div>
					<label className='mb-2 block font-satoshi text-sm font-medium text-dark dark:text-white'>Address (MN)</label>
					<textarea rows={2} value={data.address} onChange={set("address")}
						className='w-full rounded-lg border border-stroke bg-gray-1 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20 dark:border-stroke-dark dark:bg-white/5 dark:text-white' />
				</div>
				<div>
					<label className='mb-2 block font-satoshi text-sm font-medium text-dark dark:text-white'>Address (EN)</label>
					<textarea rows={2} value={data.address_en} onChange={set("address_en")}
						className='w-full rounded-lg border border-stroke bg-gray-1 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20 dark:border-stroke-dark dark:bg-white/5 dark:text-white' />
				</div>
			</div>

			<div className='grid gap-4 sm:grid-cols-2'>
				<InputGroup label='Phone' type='text' name='phone' value={data.phone} placeholder='+976 7018-7588' handleChange={set("phone") as (e: React.ChangeEvent<HTMLInputElement>) => void} />
				<InputGroup label='Email' type='email' name='email' value={data.email} placeholder='contact@barloworld.mn' handleChange={set("email") as (e: React.ChangeEvent<HTMLInputElement>) => void} />
			</div>
			...
			<div className='border-t border-stroke pt-4 dark:border-stroke-dark'>
				<button type='submit' disabled={loading}
					...
					{loading ? <><Loader style='border-white' /><span className='ml-2'>Saving…</span></> : "Save Settings"}
				</button>
			</div>
		</form>
	);
