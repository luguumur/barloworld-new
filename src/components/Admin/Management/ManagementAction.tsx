...
import { deleteManagement } from "@/actions/management";
...
export default function ManagementAction({ management }: { management: Management }) {
	...
	const handleDelete = async () => {
		setLoading(true);
		try {
			await deleteManagement(management.id);
	...
	return (
		<>
			<div className="mt-2 flex flex-wrap items-center gap-3.5 lsm:ml-auto lsm:justify-end sm:mt-0">
				<Link
					href={`/admin/management/${management.id}/edit`}
			...
			<DeleteModal
				...
				deleteText="Remove this management entry"
