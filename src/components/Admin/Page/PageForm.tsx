...
import { createPage, updatePage, type PageInput } from "@/actions/page";
...
export default function PageForm({
	...
	const handleTitleEnChange = (value: string) => {
		setData((p) => ({
			...
			title_en: value,
			...(mode === "create" &&
				!p.slug?.trim() && { slug: slugFromTitle(value) }),
		}));
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (
			...
			return toast.error(
				"Slug, Title (MN & EN), and Content (MN & EN) are required!"
			);
		}
		setLoading(true);
		try {
		...
		<div className='rounded-10 bg-white p-6 shadow-1 dark:bg-gray-dark sm:p-8'>
			...
			<form onSubmit={handleSubmit} className='flex flex-col space-y-4'>
				...
				<p className='-mt-2 text-sm text-body/70 dark:text-gray-5'>
					Page will be available at /{data.slug.trim() || "…"}. Use lowercase
