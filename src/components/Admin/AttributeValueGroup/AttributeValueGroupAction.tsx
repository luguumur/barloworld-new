...
import { deleteAttributeValueGroup } from "@/actions/attributeValueGroup";

type Item = { id: string; name: string; name_en: string };

export default function AttributeValueGroupAction({ item }: { item: Item }) {
	...
	const handleDelete = async () => {
		setLoading(true);
		try {
			await deleteAttributeValueGroup(item.id);
	...
	return (
		<>
			<div className="mt-2 flex flex-wrap items-center gap-3.5 lsm:ml-auto lsm:justify-end sm:mt-0">
				<Link
					href={`/admin/attribute-value-groups/${item.id}/edit`}
			...
			<DeleteModal
				...
				deleteText="Remove attribute group"
