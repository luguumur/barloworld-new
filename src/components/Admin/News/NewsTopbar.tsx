...
export default function NewsTopbar({
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (categoryId.trim()) params.set("category", categoryId.trim());
				...
				<form onSubmit={handleSubmit} className="flex flex-wrap items-center gap-3">
					{categories.length > 0 && (
						<select
							...
							className="h-11 rounded-lg border border-stroke bg-gray-1 px-4 outline-none ring-offset-1 duration-300 focus:shadow-input focus:ring-2 focus:ring-primary/20 dark:border-stroke-dark dark:bg-transparent dark:text-white"
						>
